...
<div class="wrap dbmig-wrap dbmig-guide">
	...
	<div class="dbmig-card" id="g-user">
		...
		<table class="dbmig-guide-table">
			...
			<tr><th>Core fields</th><td><code>user_login</code>, <code>user_email</code>, <code>user_pass</code>, <code>user_nicename</code>, <code>display_name</code>, <code>first_name</code>, <code>last_name</code>, <code>nickname</code>, <code>user_url</code>, <code>description</code>, <code>user_registered</code>.</td></tr>
...
Join 2:  LEFT JOIN  category          ON  news_categories.cat_id = category.id</pre>
	...
	<div class="dbmig-card" id="g-acf">
		...
		<ul class="dbmig-bullets">
			...
			<li><strong>ACF repeaters</strong> (Step 4): map a one-to-many <strong>child table</strong> into a repeater. Each child row becomes one repeater row; map each sub-field to a child column. Set the <em>Child FK column</em> (points back to the parent id) and optional order.</li>
		</ul>
...
WHERE image IS NOT NULL
  AND TRIM(image) &lt;&gt; '';</pre>
	...
	<div class="dbmig-card" id="g-run">
		...
		<p><strong>Typical combo for a big migration:</strong> <em>Run SQL (fast)</em> for the bulk (posts, meta, taxonomy, authors), then <em>Run import (PHP)</em> once if you also use ACF repeaters.</p>
